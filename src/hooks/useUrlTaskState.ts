...
const VALID_SORT_FIELDS: TaskSortField[] = ['dueDate', 'priority', 'createdAt', 'title'];
...
export function useUrlTaskState() {
  ...
  const filters: TaskFilterState = useMemo(() => {
    ...
    const sortBy: TaskSortField = VALID_SORT_FIELDS.includes(rawSortBy) ? rawSortBy : 'createdAt';
  ...
  const setSortBy = useCallback(
    (sortBy: TaskSortField) => {
      updateUrl({ sort: sortBy === 'createdAt' ? null : sortBy });
    },
    [updateUrl]
  );
