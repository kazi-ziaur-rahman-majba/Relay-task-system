...
export default function TeamPage() {
  ...
  // Pagination math for Team Directory (9 cards per page)
  const totalMembers = owners.length;
  const totalMemberPages = Math.max(1, Math.ceil(totalMembers / pageSize));
  const validPage = page > totalMemberPages ? 1 : page;

  const startIndex = totalMembers === 0 ? 0 : (validPage - 1) * pageSize + 1;
  const endIndex = Math.min(validPage * pageSize, totalMembers);
  ...
  return (
    <div className="min-h-full bg-[#F7F7F7] text-slate-900 font-sans antialiased">
      <main className="w-full max-w-full px-1 md:px-2 lg:px-6 py-1 md:py-2 lg:py-4 space-y-3 sm:space-y-6">
        {/* Header */}
        <TaskHeader
